    ...
    return `
      <div class="rec-card ${i === 0 ? 'rank-1' : ''}">
        ...
        <div class="comp-profile">
          <div style="text-transform: uppercase; font-size: 0.7rem; letter-spacing: 0.1em; color: var(--text-muted); margin-bottom: 0.5rem; border-bottom: 1px solid var(--border); padding-bottom: 0.2rem;">Competition Profile</div>
        ...
        <div class="comp-profile">
          <div style="text-transform: uppercase; font-size: 0.7rem; letter-spacing: 0.1em; color: var(--text-muted); margin-bottom: 0.5rem; border-bottom: 1px solid var(--border); padding-bottom: 0.2rem;">Rapor & UTBK Profile</div>
          ...
          <div class="pathways__body" id="${pwId}" style="display:none; flex-direction:column; gap:0.5rem;">
            ${pw.map(p => `
              <div class="pathway-row" style="font-size: 0.8rem; margin-bottom: 0.5rem;">
                <div style="font-weight:600;">${p.name} <span style="color:var(--text-secondary); font-weight:normal;">(${p.type})</span></div>
        ...
        <div class="career-panel" style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border);">
          <div style="text-transform: uppercase; font-size: 0.7rem; letter-spacing: 0.1em; color: var(--text-muted); margin-bottom: 0.5rem;">Career Prospects</div>
          ...
          ${ca.avg_starting_salary_idr ? `<div style="font-weight:600; color:var(--gold); margin-bottom:0.2rem;">${formatIDR(ca.avg_starting_salary_idr)} avg starting</div>` : ''}
          ${ca.salary_range ? `<div style="font-size:0.75rem;color:var(--text-secondary);margin-bottom:0.3rem">${ca.salary_range}</div>` : ''}
          ${ca.job_market_demand ? `<div style="font-size:0.8rem; margin-bottom:0.2rem;">Demand: <span class="demand-badge demand-badge--${ca.job_market_demand}" style="font-weight:600; padding:0.1rem 0.3rem; border:1px solid currentColor; border-radius:2px;">${ca.job_market_demand.toUpperCase()}</span></div>` : ''}
          ${ca.demand_rationale ? `<div style="font-size:0.8rem; color:var(--text-secondary); margin-bottom:0.2rem;">${ca.demand_rationale}</div>` : ''}
          ${ca.growth_outlook ? `<div style="font-size:0.8rem; margin-top:0.3rem;color:var(--gold)">${ca.growth_outlook}</div>` : ''}
        ...
        <div class="insight-block" style="margin-top: 1rem; padding: 1rem; background: var(--bg-muted); border-left: 3px solid var(--gold); font-size: 0.85rem;">
          <div style="font-weight:600; margin-bottom:0.5rem;">${ins.headline || 'Strategic Overview'}</div>
          ${ins.acceptance_context ? `<div style="margin-bottom:0.3rem;">${ins.acceptance_context}</div>` : ''}
          ${ins.score_context ? `<div style="margin-bottom:0.3rem;">${ins.score_context}</div>` : ''}
          ${ins.rapor_utbk_context ? `<div style="margin-bottom:0.3rem;">${ins.rapor_utbk_context}</div>` : ''}
          ${ins.trend_signal ? `<div style="margin-bottom:0.3rem;">${ins.trend_signal}</div>` : ''}
          ${ins.risk_assessment ? `<div style="color:var(--accent); margin-bottom:0.3rem;">${ins.risk_assessment}</div>` : ''}
          ${ins.strategic_note ? `<div style="font-style:italic; font-weight:600; margin-top:0.5rem;">${ins.strategic_note}</div>` : ''}
        </div>
...
function renderComparativeSummary() {
  ...
  el.innerHTML = `
    ...
    <div class="comp-summary__row">
      <div class="comp-summary__item">
        ...
        <div class="comp-summary__val">
          ...
          <span style="margin-left:0.5rem;font-size:0.65rem;color:var(--text-muted)">${cs.safer_pick_reason || ''}</span>
        </div>
      </div>
      <div class="comp-summary__item">
    ...
    ${cs.cost_comparison ? `<div style="font-size:0.65rem;color:var(--text-muted);margin-top:0.5rem">${cs.cost_comparison}</div>` : ''}
  `;
}
